  ...
  <div>
    <header class="text-gray-600 body-font border-b border-gray-300">
      ...
      >
        ...
        <nav
          class="md:mr-auto md:ml-4 md:py-1 md:pl-4  md:border-gray-400 flex flex-wrap items-center text-base justify-center"
          :class="user ? 'md:border-l' : ''"
        >
          <template v-if="user">
            <router-link
              class="mr-5 hover:text-gray-900"
              v-if="user.user_type === 'Cook'"
              to="/cook"
              >Cooks</router-link
            >
            <router-link
              class="mr-5 hover:text-gray-900"
              v-if="user.user_type === 'Patient'"
              to="/recipient"
        ...
        <button
          v-if="user"
          @click="logout()"
          class="inline-flex items-center text-white bg-button border-0 py-1 px-3 focus:outline-none hover:bg-button rounded mt-4 md:mt-0"
        >
          Logout
          <svg
            fill="none"
